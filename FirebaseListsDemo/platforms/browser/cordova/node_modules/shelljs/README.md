...
If you have feedback, suggestions, or need help, feel free to post in
our [issue tracker](https://github.com/shelljs/shelljs/issues).
...
target `docs`: `$ node make docs`.

You can also pass arguments to your targets by using the `--` separator. For example, to pass `arg1` and `arg2` to a
target `bundle`, do `$ node make bundle -- arg1 arg2`:
...
Analogous to the redirect-and-append operator `>>` in Unix, but works with JavaScript strings (such as those returned
by `cat`, `grep`, etc).
...
Reads an input string from `files` and performs a JavaScript `replace()` on the input using the given search regex and
replacement string or function. Returns the new string after replacement.
...
`PATHEXT` variable to append the extension if it's not already executable. Returns string containing the absolute path
to the command.
...
object `{ code:..., stdout:... , stderr:... }`, containing the program's
`stdout`, `stderr`, and its exit `code`. Otherwise returns the child process object, and the `callback` gets the
...
created empty, unless -c is supplied. This is a partial implementation of *[touch(1)](http://linux.die.net/man/1/touch)*
.
...
If `true` the script will die on errors. Default is `false`. This is analogous to Bash's `set -e`
